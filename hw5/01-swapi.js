...
const filterStarships = (input) => {
  ...
  const ship = [];
  for (let i in starships) {
    if (starships[i].passengers < 10 && starships[i].crew > 1) {
      ship.push(starships[i]);
    }
  }
...
const reduceStarships = (input) => {
  ...
  for (let i in starships) {
    const ship = starships[i];
    if (isNaN(Number(ship.cost_in_credits))) {
      totalCost += 0;
    } else {
      totalCost += Number(ship.cost_in_credits);
    }
  }

  return (
    "The cost of all starships is " + totalCost.toLocaleString() + " credits"
  );
};
